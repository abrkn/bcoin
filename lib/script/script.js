...
const {encoding} = bio;
const CriticalData = require('../primitives/criticaldata');
...
class Script {
  ...
  // See bool CScript::IsCriticalHashCommit() const
  isCriticalHashCommit() {
    if (this.raw.length < 38 || this.raw[0] !== opcodes.OP_RETURN || this.raw[1] !== 0x24) {
      return false;
    }

    if (this.raw.readUInt32BE(2, true) !== 0xd1617368) {
      return false;
    }
  ...
  getCriticalData() {
    assert(this.isCriticalHashCommit());

    // Skip past OP_RETURN 0x24 0xd1617368 (6 bytes)
    const hashCritical = this.raw.slice(6, 6 + 32);

    // Skip past previous
    const bytes = this.raw.slice(6 + 32);

    return CriticalData.fromRaw(bytes, hashCritical);
  }
