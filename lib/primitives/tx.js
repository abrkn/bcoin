...
class TX {
  ...
  frameNormal() {
    return this.frameWitness(false);
  }
  ...
  frameWitness(includeWitness) {
    const raw = this.getWitnessSizes(includeWitness);
    const bw = bio.write(raw.size);
    this.writeWitness(bw, includeWitness);
    raw.data = bw.render();
    return raw;
  }
  ...
  // https://github.com/DriveNetTESTDRIVE/DriveNet/blob/a0314676bffea5de962b15320babc699967d2f79/src/primitives/transaction.h#L285
  writeWitness(bw, allowWitness = true) {
    // console.trace('--- serializing tx', { allowWitness });
  ...
  getNormalSizes() {
    return this.getWitnessSizes(false);
  }
  ...
  getWitnessSizes(includeWitness) {
    const sizeNormal = this.writeWitness(bio.write(1024 * 1024), false).slice().length;
    const sizeWithWitness = includeWitness ? this.writeWitness(bio.write(1024 * 1024), true).slice().length: sizeNormal;

    return new RawTX(sizeWithWitness, sizeNormal);
  }
