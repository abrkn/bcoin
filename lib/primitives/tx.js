...
const CriticalData = require('./criticaldata');
...
class TX {
  ...
  fromReader(br) {
    const spam = (..._) => TX.spam && console.log('SPAM', ..._);

    // TODO
    const allowCriticalData = true;
    const allowWitness = true;
    ...
    function readInputs() {
      ...
      for (let i = 0; i < inCount; i++)
        inputs.push(Input.fromReader(br));
    ...
    spam('version', this.version);

    if (this.version === 3) {
      this.replayBytes = br.readU8();
      spam('replayBytes', this.replayBytes);
    }
    ...
    if (this.inputs.length === 0 && allowWitness) {
      // We read a dummy or an empty vin.
      spam('read dummy vin');

      flags = br.readU8();

      spam('flags', flags, '1?', flags & 1, '2?', flags & 2);

      if (flags !== 0) {
        this.inputs = readInputs();
        this.outputs = readOutputs();
      }
    } else {
      // We read a non-empty vin. Assume a normal vout follows.
      this.outputs = readOutputs();
      spam('tx is normal and boring');
    }
    ...
    if (flags & 1 && allowWitness) {
      spam('lets read da witness things');
      // The witness flag is present, and we support witnesses.
      flags = flags ^ 1;

      witness = br.offset;

      // spam('loop over', this.inputs.length, 'ins');
      ...
      spam('has witness?', hasWitness);
      // spam('first?', this.inputs.length && this.inputs[0])

      witness = (br.offset - witness) + 2;
    }

    if (flags & 2 && allowCriticalData) {
      // The critical data flag is present, and we support critical data.
      flags = flags ^ 2;
      this.criticalData = CriticalData.fromReader(br);
      spam('awwww shit it has CRIT data');
    }
    ...
    if (!this.mutable) {
      spam('storing state in TX');
      this._raw = br.endData();
      this._size = this._raw.length;

      spam('hasWitness?', hasWitness);
  ...
  // See https://github.com/DriveNetTESTDRIVE/DriveNet/blob/a0314676bffea5de962b15320babc699967d2f79/src/primitives/transaction.h#L243
  writeWitness(bw, allowWitness = true) {
    const spam = (..._) => TX.spam && console.log('SPAM WRITE', ..._);

    spam('lets write...');
    spam(new Error().stack);
    spam('allowWitness?', allowWitness);

    spam('expecting to write', bw.data && bw.data.length, 'bytes');
    ...
    if (this.version === 3) {
      assert(this.replayBytes);
      bw.writeU8(this.replayBytes);
    }
    ...
    if (flags & 1) {
      ...
      witness = bw.offset - start;
    }

    if (flags & 2) {
      this.criticalData.toWriter(bw);
    }

    bw.writeU32(this.locktime);

    if (witness !== undefined && witness === this.inputs.length)
      throw new Error('Cannot serialize empty-witness tx.');

    spam('k im done writign and wrote like', bw.offset, 'bytez');
  ...
  getNormalSizes() {
    ...
    if (flags & 2) {
      base += this.criticalData.getSize();
    }
  ...
  getWitnessSizes() {
    ...
    for (const output of this.outputs) {
      TX.spam && console.log('*** output size for write', output.getSize());
      base += output.getSize();
    }
