...
class Block extends AbstractBlock {
  ...
  hasWitness() {
    console.log('has witness? lets look at');

    if (this._witness !== -1) {
      console.log('some cached value', this._witness);
      return this._witness !== 0;
    }

    for (const tx of this.txs) {
      if (tx.hasWitness())
        console.log('yes', tx.rhash(), 'does');
        return true;
    }
  ...
  checkBody() {
    ...
    if (!this.merkleRoot.equals(root)) {
      console.log('merkle root (block.js)');
      console.log('block     ', this.rhash('hex'));
      console.log('root      ', root);
      console.log('merkelRoot', this.merkleRoot);
