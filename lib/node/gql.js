/*!
 * gql.js - GraphQL server for bcoin.
...
class GQL {
  ...
  constructor(node) {
    assert(node, 'GQL requires a Node.');
    ...
    this.pool = node.pool;
    this.fees = node.fees;
    this.logger = node.logger.context('gql');
  ...
  init() {
    ...
    const formatScript = (script) => {
      return script.toJSON().toString();
    };

    const formatCriticalData = (criticalData) => {
      return {
        bytes: criticalData.bytes.toString('hex'),
        hashCritical: criticalData.hashCritical.toString('hex')
      };
    };

    const formatOutpoint = (outpoint) => {
      return {
    ...
    const formatOutput = (output) => {
      return {
        value: output.value,
        script: formatScript(output.script)
      };
    };

    const formatInput = (input) => {
      return {
        script: formatScript(input.script),
        outpoint: input.prevout ? formatOutpoint(input.prevout) : null
      };
    };
    ...
    const formatTx = (tx) => {
      return {
        ...
        inputs: tx.inputs.map(formatInput),
        outputs: tx.outputs.map(formatOutput),
        criticalData: tx.criticalData ? formatCriticalData(tx.criticalData) : null
      };
    };

    const formatBlock = (block, entry) => {
      return {
        hash: block.rhash('hex'),
        height: entry.height,
        prevBlockHash: entry.prevBlock.toString('hex'),
        powHash: block.powHash('hex'),
        txs: block.txs ? block.txs.map(formatTx) : null
      };
    };
    ...
    this.rootResolver = {
      block: async (args) => {
        ...
        return formatBlock(block, entry);
      }
    };
    ...
    this.app.use((error, req, res, next) => console.error(error.stack) || next(error));
